...
export function ComponentPickerMenuPlugin({ baseOptions = [], dynamicOptionsFn }: { baseOptions?: Array<ComponentPickerOption>; dynamicOptionsFn?: ({ queryString }: { queryString: string }) => Array<ComponentPickerOption> }): JSX.Element {
    ...
    const options = useMemo(() => {
        ...
        return [...(dynamicOptionsFn?.({ queryString }) || []), ...baseOptions.filter((option) => regex.test(option.title) || option.keywords.some((keyword) => regex.test(keyword)))];
