...
export const ThemeToggleButton = ({ theme = "light", showLabel = false, variant = "circle", start = "center", url, className, onClick }: ThemeToggleButtonProps) => {
	...
	return (
		<Button variant="outline" size={showLabel ? "default" : "icon"} onClick={handleClick} className={cn("relative overflow-hidden transition-all", showLabel && "gap-2", className)} aria-label={`Switch to ${theme === "light" ? "dark" : "light"} theme`}>
