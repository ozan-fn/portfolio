		...
		function $$render_inner($$renderer3) {
			...
			$$renderer3.push(`<!----> `);
			{
				let main = function($$renderer4) {
					...
					$$renderer4.push(`<!----> <select id="status" name="status" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">`);
					$$renderer4.option({ value: "COMPLETED" }, ($$renderer5) => {
						$$renderer5.push(`Completed`);
					});
					$$renderer4.option({ value: "IN_PROGRESS" }, ($$renderer5) => {
						$$renderer5.push(`In Progress`);
					});
					$$renderer4.option({ value: "ARCHIVED" }, ($$renderer5) => {
						$$renderer5.push(`Archived`);
					});
					$$renderer4.push(`</select></div> <div class="grid gap-2 pt-2">`);
					Label($$renderer4, {
						for: "env",
						children: ($$renderer5) => {
...
//# sourceMappingURL=_page.svelte-yKy_o60P.js.map