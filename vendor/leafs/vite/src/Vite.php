...
class Vite
{
     ...
     * @param  array|string  $config The config array or the config key
     * @param  mixed  $value The config value if $config is a string
     */
    public static function config($config, $value = null)
    {
        if (is_array($config)) {
            static::$paths = array_merge(static::$paths, $config);
        } else {
            static::$paths[$config] = $value;
        }
    }
