...
<div class="relative w-full h-full min-h-[70vh] flex flex-col items-center justify-center p-4">
    <div class="grid grid-cols-1 grid-rows-1 w-full max-w-md">
        {#if currentView === 'menu'}
            <div class="col-start-1 row-start-1 w-full" in:fly={{ y: 20, duration: 400, delay: 200, easing: cubicOut }} out:fade={{ duration: 200 }}>
                ...
                <Card class="w-full text-center">
                    ...
                    <CardContent>
                        <div class="flex flex-col gap-2 text-left mb-6">
                            ...
                            <div class="bg-muted/50 p-3 rounded-lg flex flex-col gap-2 min-h-16">
                        ...
                        {:else}
                            <p class="text-sm text-muted-foreground animate-pulse">Menunggu Host memulai permainan...</p>
                        {/if}
                    </CardContent>
                </Card>
            </div>
        ...
        {#if currentView === 'playing'}
            <div class="col-start-1 row-start-1 w-full flex flex-col gap-6" in:fly={{ y: 20, duration: 400, delay: 200, easing: cubicOut }} out:fade={{ duration: 200 }}>
                ...
                <div class="flex flex-col items-center gap-2">
                    ...
                    <div class="w-full h-48 rounded-xl shadow-inner transition-opacity duration-500 flex items-center justify-center relative" style="background-color: hsl({targetColor.h}, {targetColor.s}%, {targetColor.l}%); opacity: {showTarget ? '1' : '0'};">
                        ...
                        {/if}
                        {#if !showTarget}
                            <span class="absolute inset-0 flex items-center justify-center text-muted-foreground font-mono" in:fade>Warna Disembunyikan</span>
                        {/if}
                    </div>
                </div>
            ...
            <div class="col-start-1 row-start-1 w-full" in:fly={{ y: 20, duration: 400, delay: 200, easing: cubicOut }} out:fade={{ duration: 200 }}>
                <Card class="w-full text-center">
                    ...
                    <CardContent class="flex flex-col gap-6">
                        ...
                        <div in:scale={{ delay: 300, duration: 500, start: 0.8, opacity: 0, easing: cubicOut }}>
                        ...
                        {#if client}
                            <div class="bg-muted p-4 rounded-lg text-left border">
                                ...
                                {#each [...players].sort((a, b) => b.score - a.score) as player, i}
                                    <div class="flex justify-between items-center text-sm py-1.5 border-b border-border/50 last:border-0" in:slide>
                                        <span>
        ...
        {#if currentView === 'final_result'}
            <div class="col-start-1 row-start-1 w-full" in:fly={{ y: 20, duration: 500, delay: 200, easing: cubicOut }} out:fade={{ duration: 200 }}>
                    ...
                    <CardContent class="flex flex-col gap-6">
                        {#if client}
                            <div class="bg-muted p-4 rounded-xl text-left border shadow-inner">
                                ...
                                {#each [...players].sort((a, b) => b.score - a.score) as player, i}
                                    <div class="flex justify-between items-center text-sm py-2.5 border-b last:border-0 border-border/50" in:slide={{ delay: 200 + i * 150 }}>
                        ...
                        <div class="flex flex-col gap-3 mt-4">
                            ...
                            {:else}
                                <p class="text-sm text-muted-foreground bg-muted p-3 rounded-lg border animate-pulse">Menunggu Host untuk memulai Rematch...</p>
                            {/if}
                            <Button
                                variant="ghost"
                                onclick={() => {
