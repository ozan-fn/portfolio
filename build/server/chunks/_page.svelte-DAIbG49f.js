				...
				if (Card) {
					...
					Card($$renderer, {
						...
						children: ($$renderer) => {
							...
							if (Aspect_ratio) {
								...
								Aspect_ratio($$renderer, {
									...
									children: ($$renderer) => {
										if (cert.thumbnail) {
											$$renderer.push("<!--[0-->");
											$$renderer.push(`<img${attr("src", cert.thumbnail)}${attr("alt", cert.title)} class="object-cover w-full h-full transition-transform duration-700 group-hover:scale-105"/>`);
										} else {
											$$renderer.push("<!--[-1-->");
							...
							$$renderer.push(` <div class="absolute top-3 left-3"><span class="text-[9px] font-bold tracking-widest uppercase px-2.5 py-1 rounded-md backdrop-blur-md border border-primary/20 bg-background/80 text-foreground">${escape_html(cert.issuer)}</span></div></div> <div class="p-5 flex flex-col flex-1 gap-4"><div class="flex-1 space-y-2"><h4 class="text-lg font-bold tracking-tight text-foreground line-clamp-2 leading-snug group-hover:text-primary transition-colors">${escape_html(cert.title)}</h4> <div class="flex items-center gap-1.5 text-muted-foreground text-[10px] font-bold uppercase tracking-widest">`);
							Calendar($$renderer, {
								size: 12,
								class: "opacity-70"
							});
...
//# sourceMappingURL=_page.svelte-DAIbG49f.js.map